...
$uiRoot = [System.IO.Path]::Combine($path, 'Dpz.Core.WebMore')
$cssSourcePath = [System.IO.Path]::Combine($uiRoot, 'wwwroot', 'css')
...
try {
    ...
    Write-Host "  Output: Dpz.Core.WebMore/wwwroot/css/$outputFile" -ForegroundColor green
...
function Update-IndexHtml($indexHtmlPath) {
    if (Test-Path $indexHtmlPath) {
        Write-Host "Updating $indexHtmlPath..." -ForegroundColor cyan
        
        $indexContent = Get-Content $indexHtmlPath -Raw -Encoding UTF8
        # 匹配任何 global.min 开头的 CSS 文件引用（RCL 路径）
        $pattern = '<link href="_content/Dpz.Core.WebMore/css/global\.min\.[^"]*\.css" rel="stylesheet" />'
        $replacement = '<link href="_content/Dpz.Core.WebMore/css/{0}" rel="stylesheet" />' -f $outputFile
...
Update-IndexHtml ([System.IO.Path]::Combine($path, 'Dpz.Core.WebMore.SPA', 'wwwroot', 'index.html'))
Update-IndexHtml ([System.IO.Path]::Combine($path, 'Dpz.Core.WebMore.App', 'wwwroot', 'index.html'))
