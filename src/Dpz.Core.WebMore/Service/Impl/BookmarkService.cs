using System.Collections.Generic;
using System.Linq;
using System.Net;
using System.Net.Http;
using System.Net.Http.Json;
using System.Threading.Tasks;
using Dpz.Core.WebMore.Models;

namespace Dpz.Core.WebMore.Service.Impl;

public class BookmarkService(HttpClient httpClient) : IBookmarkService
{
    public async Task<List<BookmarkModel>> GetBookmarksAsync(
        string? title,
        ICollection<string> categories
    )
    {
        var parameterJoin = GetParameters(title, categories);
        return await GetDataAsync<BookmarkModel>("/api/Bookmark" + parameterJoin);
    }

    public async Task<List<string>> GetCategoriesAsync()
    {
        return await GetDataAsync<string>("/api/Bookmark/categories");
    }

    public async Task<List<string>> SearchAsync(string title, ICollection<string> categories)
    {
        var parameterJoin = GetParameters(title, categories);
        return await GetDataAsync<string>("/api/Bookmark/search" + parameterJoin);
    }

    private static string GetParameters(string? title, ICollection<string> categories)
    {
        var parameters = new List<string>();
        if (!string.IsNullOrEmpty(title))
        {
            parameters.Add($"title={WebUtility.UrlEncode(title)}");
        }

        if (categories is { Count: > 0 })
        {
            parameters.AddRange(
                categories
                    .Where(x => !string.IsNullOrEmpty(x))
                    .Distinct()
                    .Select(category => $"categories={WebUtility.UrlEncode(category)}")
            );
        }

        var parameterJoin = parameters.Count > 0 ? "?" + string.Join("&", parameters) : "";
        return parameterJoin;
    }

    private async ValueTask<List<T>> GetDataAsync<T>(string requestUri)
    {
        var data = await httpClient.GetFromJsonAsync<List<T>>(requestUri) ?? [];
        return data;
    }
}
